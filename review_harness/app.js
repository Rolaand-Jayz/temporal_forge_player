...
const FALLBACK_PAIRS=[[360,720],[360,1080],[480,720],[480,1080],[720,1080],[720,1440],[1080,1440]];
...
const SCENES=[['tos_daylight','Tears of Steel · daylight'],['sintel_cave','Sintel · cave']];
const VIEW_MODES=[['delivery','Delivery image']];
const PATHS=[['current','Temporal Forge · baseline'],['bilinear','Baseline + CAS 0.20'],['lanczos','Conventional Lanczos'],['bicubic','Conventional cubic']];
const METHODS=['current_cas20','base_only_bilinear_cas20','conventional_lanczos','conventional_bicubic'];
...
const state={left:{scene:'tos_daylight',frame:Number(CAMPAIGN.frame||48),input:720,output:1080,path:'current',view:'delivery'},right:{scene:'tos_daylight',frame:Number(CAMPAIGN.frame||48),input:720,output:1080,path:'lanczos',view:'delivery'},split:50,zoom:100};
...
function workingGrid(selection){
  ...
  return`${deliveryWidth(selection.output)}×${selection.output}`;
}
...
function renderPanel(side){
  ...
  const advanced='';
  const viewModes=VIEW_MODES;const target=$(`${side}-panel`);
