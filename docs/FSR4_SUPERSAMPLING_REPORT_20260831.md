...
the paired temporal data covers all five multipliers. Temporal mean SSIM /
temporal-delta signal were 0.880951 / 6.133414, 0.824979 / 6.985770,
0.835283 / 6.920159, 0.834618 / 6.840943, and 0.827792 / 6.725866 for 2.00x
through 3.00x respectively. Spatial mean SSIM across the five scales was
0.891968, 0.892311, 0.892365, 0.892589, and 0.892540 respectively. This is
mixed 2160p evidence: the direct 2.00x delivery-grid arm is strongest on the
temporal aggregate, and the result does not support a universal 3.00x claim.
Clean performance isolation remains open.

## Resolution coverage and label audit

The portable harness retains all required controls: input heights 360, 480, 540,
720, and 1080, plus output heights 720, 1080, 1440, and 2160. The new
`benchmarks/quality_sweeps/resolution_480_to_720_20260901.csv` adds 20 serial
rows for the native 854x480 corpus tier, covering all four real scenes and all
five reconstruction ratios at 1280x720 delivery. The output tiers 1080p,
1440p, and 2160p remain covered by their existing paired matrices. The corpus
contains no native 540p high-quality tier, so 540p remains an honest `NO IMAGE`
control in the harness rather than a mislabeled proxy capture.

The companion
`benchmarks/quality_sweeps/resolution_480_to_720_20260901.audit.json` was
generated by `tools/audit_quality_evidence.py`. All 20 rows matched their
scene, frame, source/intermediate/output labels, captured asset, and source
frame timestamp. This is the end-of-run check for future sweeps as well as the
new 480p slice; older rows remain supported by their recorded frame and hash
fields, but do not contain wall-clock capture timestamps.
...
| 16 | The 3.00x aggregate direction repeats at 1440p, while the complete 2160p matrix favors the direct 2.00x delivery-grid arm temporally. Neither establishes a universal ratio. | 1440p and complete 2160p paired coverage supported; clean performance isolation remains open. |
