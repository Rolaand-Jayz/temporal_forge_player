...
The harness now contains 125 validated frame-48 PNGs covering the four real
scenes across the five reconstruction scales and 720p/1080p/1440p/2160p
delivery sets, finalist controls, NativeAA/conventional downscale controls,
and CAS-placement controls. Other legal combinations remain selectable and
...
## 2026-09-02 — quality-campaign recovery: evidence preserved, capture invalidated

The harness recapture at
`benchmarks/quality_sweeps/harness_campaign_recapture_20260902_cas20_release_final/`
was invalidated after runtime evidence showed that `review_pipeline.env`
overrode the requested CAS `.20` with `.04`. Its image payloads were removed
under the accepted data-only policy, while CSVs, JSON manifests, runtime
traces, logs, hashes, and invalidation metadata were retained. The explicit
invalidation record is `INVALIDATED.json`; this root must not supply harness
images or quality conclusions.

The runner now preserves caller-owned CAS values across the review-config
boundary and validates the effective state emitted by Temporal Forge. A
controlled smoke run first reproduced the `.04` defect; the corrected run
reported CAS `0.20` at `integrated_post_reconstruction`. The runner now has
separate `AMD_SEMANTIC_BASELINE` and `JITTER_OFF_DIAGNOSTIC` profiles. The
baseline smoke trace proved prepass input-resolve jitter, source-tap μ-law,
history, recurrent state, unjittered motion lookup, motion validity, and CAS
`.20`; the failed intermediate smoke trace is retained as diagnostic evidence.

The data-only historical classification is generated by
`tools/audit_quality_campaign_evidence.py` and recorded in
`benchmarks/quality_sweeps/quality_campaign_evidence_audit_20260902.json` and
its Markdown companion. At this point every canonical harness method remains
`RECAPTURE REQUIRED`; no invalid or incomplete prior image is being promoted.
The corrected full campaign and qualification suite remain gated on completing
the remaining runtime semantic and provenance checks.

The retention policy is now explicit: temporal capture runners use image/video
files only as temporary metric inputs and retain data artifacts by default
(CSV/JSON manifests and sidecars, hashes, logs, metadata, and traces). A
caller must set `TFORGE_PRESERVE_IMAGE_ARTIFACTS=1` to retain image/video
payloads for a review-harness build. The data-only failure-path contract is
covered by `tests/test_temporal_runner_contract.py`.

The five-case AMD semantic qualification is captured in
`benchmarks/quality_sweeps/baseline_qualification_20260902_data_only_final/`.
It uses matched source/reference media for static high detail, smooth pan,
independent motion, occlusion/disocclusion, and a generated hard cut. All
five cases passed runtime-trace validation with jitter enabled, unjittered
motion lookup, explicit source-to-motion mapping, history/recurrent state,
resolve CAS `.20`, and frame-aligned event traces. The hard-cut case recorded
later detector events and resets. The result contains data artifacts only;
the authoritative binary SHA is `6015842b5092c31673123ffc923f95f2a9cbf1adc15dedf634241af8d14a0d1b`,
captured against `0ced5f210a3b9167d74027d2578cd7340a7eb36b` with a dirty tree.
This is qualification evidence, not yet the final clean-tree campaign.
