...
def periodic_energy(image: np.ndarray) -> tuple[float, list[dict[str, float]]]:
    ...
    peaks = []
    for y, x in zip(*np.unravel_index(np.argsort(spectrum.ravel())[-8:], spectrum.shape)):
    ...
    return float(spectrum.sum() / (gray.size * gray.size)), peaks
...
def main() -> int:
    ...
    def summarize(a: np.ndarray, b: np.ndarray) -> dict:
        delta = a - b
        energy, peaks = periodic_energy(delta)
        return {"rmse": float(np.sqrt(np.mean(delta * delta))),
                "max_abs": float(np.max(np.abs(delta))),
                "periodic_energy": energy, "dominant_periods": peaks}
    ...
    result = {
        ...
        "carrier": {
            ...
            "interpretation": "No periodic residual amplification was observed in the quantized carrier comparison; this is an offline ablation, not a production carrier swap.",
        },
        "transform_order": {
            "transform_before_resolve_vs_resolve_before_transform": summarize(transform_before, resolve_before),
            "interpretation": "Ordering is numerically distinguishable, but the independent GPU resolve-before-transform arm remains outside the current descriptor contract.",
        },
    }
