...
const resultsRoot = path.resolve('benchmarks/video_corpus/results');
...
const excludedReviewAsset = name => {
  return /^(?:synthetic(?:_|\.)|source(?:_|\.)|supersampled_aa(?:_|\.)|intel_|os_|s_)/i.test(name) || /lanczos_roundtrip_fsr/i.test(name) || /frame55_/i.test(name) || /reference_.*_f55\.png$/i.test(name) || /_(?:difference|metadata)\.png$/i.test(name);
};
...
for (const [name, sourcePath] of [...sourceByName.entries()].sort(([a], [b]) => a.localeCompare(b))) {
  const key = `${assetFamily(name)}:${createHash('sha1').update(fs.readFileSync(sourcePath)).digest('hex')}`;
  if (!uniqueSources.has(key)) uniqueSources.set(key, [name, sourcePath]);
}
sourceByName.clear();
for (const [name, sourcePath] of uniqueSources.values()) sourceByName.set(name, sourcePath);
const externalAssets = [...sourceByName.keys()].sort().map(name => ({name, src: `${path.basename(assetOutputRoot)}/${name}`, imageResolution: pngDimensions(sourceByName.get(name))}));
...
<body>
  ...
  <script>
    ...
    const ASSET_RE = /^(?<scene>tos_daylight|tos_debris|bbb_grass|bbb_branches|sintel_rooftop|sintel_cave|intel_rooftop|intel_cave|os_daylight|os_debris|s_daylight|source|supersampled_aa)(?:_(?<size>\d+x\d+))?.*\.png$/;
    ...
    const techniqueLabels = {'native-input':'Native input','native-reference':'Native reference','temporal-forge':'Current Temporal Forge','bicubic-prefsr-bicubic':'Bicubic → Temporal Forge → Bicubic','bicubic-prefsr-lanczos':'Bicubic → Temporal Forge → Lanczos','lanczos-prefsr':'Lanczos → Temporal Forge','experimental':'Experimental output','lanczos-control':'Lanczos reference','lanczos-native-fsr':'Native Lanczos pass → FSR','bicubic-control':'Bicubic reference','bilinear-control':'Bilinear reference'};
    const realName = name => !/^(?:synthetic(?:_|\.)|source(?:_|\.)|supersampled_aa(?:_|\.)|intel_|os_|s_)/i.test(name) && !/_(?:difference|metadata)\.png$/i.test(name);
    ...
    function parseAssets(){
      const assets=[];
      const sources=[...Object.entries(embeddedAssets).map(([name,src])=>({name,src,embedded:true})),...externalAssets.map(x=>({...x,embedded:false}))];
      const seen=new Set();
      for(const {name,src,embedded,imageResolution:knownImageResolution} of sources){
        if(!realName(name)) continue;
        if(seen.has(name)) continue; seen.add(name);
        const m=name.match(ASSET_RE); if(!m) continue;
        const reference=Boolean(name.match(/_reference(?:_\d+x\d+)?_f\d+\.png$/));
        const target=(name.match(/_to(\d+x\d+)/)||[])[1]??null;
        const lower=name.toLowerCase();
        const imageResolution=knownImageResolution??target??m.groups.size??'unknown';
        const inputMatch=name.match(/_(\d+x\d+)(?:_|\.)/);
        const nativeInput=!reference&&!target&&!lower.includes('stage')&&!lower.includes('_current')&&!lower.includes('post-campaign')&&(/_frame\d+\.png$/i.test(name)||/_high_crf\d+_review\.png$/i.test(name))&&imageResolution===inputMatch?.[1];
        const currentName=!lower.includes('_bicubic_prefsr_')&&!lower.includes('_lanczos_prefsr_')&&(lower.includes('_current')||(lower.includes('stagea-')&&lower.endsWith('-01-current.png'))||lower.endsWith('_f48.png')||lower.endsWith('_temporal_forge_review.png'));
        const experimentalName=!currentName&&(lower.includes('stagea-')||lower.includes('base_only')||lower.includes('direct_blend')||lower.includes('stageb-')||lower.includes('stagec-')||lower.includes('staged-')||lower.includes('stagee')||lower.includes('stagef')||lower.includes('post-campaign'));
    const technique=reference?'native-reference':nativeInput?'native-input':experimentalName?'experimental':(/_bicubic_prefsr_temporal_forge_bicubic_review\.png$/i.test(lower))?'bicubic-prefsr-bicubic':(/_bicubic_prefsr_temporal_forge_lanczos_review\.png$/i.test(lower))?'bicubic-prefsr-lanczos':(/_lanczos_prefsr_temporal_forge_review\.png$/i.test(lower))?'lanczos-prefsr':currentName?'temporal-forge':(/_lanczos_native_fsr_review\.png$/i.test(lower))?'lanczos-native-fsr':(/_lanczos(?:_review)?\.png$/i.test(lower))?'lanczos-control':(/_bicubic(?:_review)?\.png$/i.test(lower))?'bicubic-control':(/_bilinear(?:_review)?\.png$/i.test(lower))?'bilinear-control':'experimental';
        const inputResolution=inputMatch?.[1]??(reference?((name.match(/_reference_(\d+x\d+)/)||[])[1]??imageResolution):imageResolution);
        const outputResolution=target??(reference?inputResolution:imageResolution);
        const featureStack=[]; if(technique==='experimental'){if(lower.includes('base_only')||lower.includes('baseonly')||lower.includes('stable_base')||lower.includes('stageb-'))featureStack.push('stable-base');if(lower.includes('learned_only'))featureStack.push('learned-only');if(lower.includes('direct_blend')||lower.includes('blend'))featureStack.push('learned-blend');if(lower.includes('stagec-')||lower.includes('stagef-res')||lower.includes('-res-')||lower.includes('_res-')||lower.includes('residual'))featureStack.push('detail-residual');if(lower.includes('adaptive')||lower.includes('sharpen'))featureStack.push('adaptive-sharpen');if(lower.includes('exposure_')||lower.includes('tone_')||lower.includes('gamma_'))featureStack.push('tone');if(!featureStack.length)featureStack.push('experimental');}
        const base={scene:m.groups.scene,inputResolution,src,assetName:name,outputResolution,imageResolution,technique,featureStack:featureStack.sort(),featureStackKey:featureStack.join('+'),baseFilter:null,blendStrength:null,learnedStrength:null,residualStrength:null,sharpeningMode:null,sharpeningStrength:null,toneConfiguration:null,experimentId:null};
        if(lower.includes('mitchell'))base.baseFilter='Mitchell'; else if(lower.includes('catmull_rom'))base.baseFilter='Catmull-Rom'; else if(lower.includes('lanczos2'))base.baseFilter='Lanczos2'; else if(lower.includes('bilinear'))base.baseFilter='Bilinear';
        const strength=lower.match(/(?:direct_blend|blend|residual|adaptive_s|sharpen_s)(?:_|-)?(\d{3})/); if(strength){const value=Number(strength[1])/1000;if(lower.includes('direct_blend'))base.learnedStrength=value; else if(lower.includes('blend'))base.blendStrength=value; else if(lower.includes('residual'))base.residualStrength=value; else {base.sharpeningMode='Adaptive';base.sharpeningStrength=value;}}
        if(lower.includes('adaptive'))base.sharpeningMode='Adaptive';
        const tone=lower.match(/(?:exposure_|tone_|gamma_)(m|p)?(\d{3})/); if(tone)base.toneConfiguration=__BT____DOLLAR__{tone[1]==='m'?'-':tone[1]==='p'?'+':''}__DOLLAR__{Number(tone[2])/1000}__BT__;
        base.experimentId=reference||technique==='native-input'?null:name.replace(/^.*(?:frame|f)48_?/,'').replace(/\.png$/,'')||null;
        assets.push(base);
      }
      return assets.map(a=>({...a,id:assetId(a)}));
    }
    ...
    function stackOptions(side){const current=byId.get(state[side]);if(!current)return[];const pool=assets.filter(a=>a.scene===current.scene&&a.inputResolution===current.inputResolution&&a.outputResolution===current.outputResolution&&a.technique==='experimental');const choices=unique(assets,'featureStackKey').filter(Boolean).flatMap(key=>key.split('+')).filter((value,index,array)=>array.indexOf(value)===index);const order=['stable-base','learned-only','learned-blend','detail-residual','adaptive-sharpen','tone','experimental'];choices.sort((a,b)=>(order.indexOf(a)-order.indexOf(b))||a.localeCompare(b));return choices.map(feature=>({value:feature,label:featureLabels[feature]??feature,available:pool.some(a=>a.featureStackKey.split('+').includes(feature))}));}
    function makeStackButtons(side){const wrap=document.createElement('div');wrap.className='field stack-field';const l=document.createElement('label');l.textContent='Experimental feature stack';const group=document.createElement('div');group.className='state-buttons';group.setAttribute('role','group');group.setAttribute('aria-label',__BT__\__DOLLAR__{side} Experimental feature stack__BT__);const current=byId.get(state[side]);const pool=assets.filter(a=>a.scene===current.scene&&a.inputResolution===current.inputResolution&&a.outputResolution===current.outputResolution&&a.technique==='experimental');const choices=unique(assets,'featureStackKey').filter(Boolean).flatMap(key=>key.split('+')).filter((value,index,array)=>array.indexOf(value)===index).sort();for(const feature of choices){const active=current.featureStack.includes(feature);const next=[...new Set(active?current.featureStack.filter(x=>x!==feature):[...current.featureStack,feature])].sort();const valid=pool.some(a=>a.featureStackKey===next.join('+'));const button=document.createElement('button');button.type='button';button.className='state-button';button.textContent=featureLabels[feature]??feature;button.setAttribute('aria-pressed',String(active));button.disabled=!valid;button.title=valid?'Toggle this feature in the stack':'No captured asset for this stack in the current context';button.addEventListener('click',()=>{if(valid){reconcile(side,{featureStackKey:next.join('+')});render();}});group.append(button)}wrap.append(l,group);return wrap;}
...
const rendered = html.replaceAll('__BT__','`').replaceAll('\\__DOLLAR__{','${').replaceAll('__DOLLAR__{','${').replace('__EMBEDDED_ASSETS__', JSON.stringify(embeddedAssets));
